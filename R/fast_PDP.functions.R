...

{
  ...
  if (computeMode$useR) {
    ...
    subset_log.ss.mt <- array(,c((parm$clust$G+1), length(parm$col.subset.I)))

    for (gg in 0:parm$clust$G)
    {subset_log.ss.mt[(gg+1),] <- PDP_fn.log.lik(gg, x.mt=parm$X[,parm$col.subset.I], parm, colSums=TRUE)
    }
    ...
    relative_I <- 1:length(parm$col.subset.I)
    ...
    {tmp <- PDP_fn.nbhd(relative_I, parm, max.col.nbhd.size)
     ...
     relative_I <- tmp[[3]]
     #
     parm$clust$col.nbhd <- c(parm$clust$col.nbhd, list(parm$col.subset.I[relative_I.k]))
     parm$clust$col.nbhd.k <- c(parm$clust$col.nbhd.k, parm$col.subset.I[relative_k])
    }
